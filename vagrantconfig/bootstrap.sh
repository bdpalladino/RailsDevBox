...
then
	...
	su - vagrant -c "ruby-install ruby 2.2.1 -- --disable-install-rdoc"
...
su - vagrant -c "source /usr/local/share/chruby/chruby.sh; chruby ruby-2.2.1; gem install rails -v 4.2.0 --no-rdoc --no-ri"
su - vagrant -C "source /usr/local/share/chruby/chruby.sh; chruby ruby-2.2.1; gem install sqlite3 -v '1.3.10' --no-rdoc --no-ri"
